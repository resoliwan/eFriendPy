from eFriendPy.Core import StandardAPIWrapper
import pandas as pd
...
class HighLevelAPI:
    ...
    def __init__(self):
        ...
        self.logger = lambda msg: print(
            msg
        )  # 사용자가 logger를 외부에서 수정할 수 있도록함(ex> slack 연동 등)
    ...
    def GetProcessedUSOrders(self, account, marketCode, startDate=None):
        ...
        res = pd.DataFrame(
            index=range(recordCount),
            columns=["주문일자", "주문번호", "원주문번호", "상품번호", "매수매도구분코드명", "주문수량", "체결단가"],
        )
    ...
    def GetProcessedKROrders(self, account, startDate=None):
        ...
        res = pd.DataFrame(
            index=range(recordCount),
            columns=["주문일자", "주문번호", "원주문번호", "상품번호", "매수매도구분코드명", "주문수량"],
        )
        for recordIdx in range(recordCount):
            date = self._core.GetMultiData(0, recordIdx, 0, 0)
            orderNum = self._core.GetMultiData(0, recordIdx, 2, 0)
    ...
    def GetUnprocessedKROrders(self, account):
        ...
        res = pd.DataFrame(
            index=range(recordCount),
            columns=["주문번호", "원주문번호", "상품번호", "매수매도구분코드명", "주문수량"],
        )
        for recordIdx in range(recordCount):
    ...
    def GetKRStocks(self, account):
        ...
        recordCount = self._core.GetMultiRecordCount(0)
        stocks = pd.DataFrame(
            index=range(recordCount), columns=["종목코드", "종목명", "현재가", "보유수량", "평가금액"]
        )
    ...
    def GetUSStocks(self, account):
        ...
        stocks = pd.DataFrame(
            index=range(recordCount),
            columns=["해외거래소코드", "종목코드", "종목명", "현재가", "보유수량", "평가금액"],
        )
